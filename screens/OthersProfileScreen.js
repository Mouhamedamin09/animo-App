...
export default function EnhancedProfileScreen() {
  ...
  const handleFollow = async () => {
    try {
      const token = await AsyncStorage.getItem('token');
      if (!token) {
        Alert.alert('Authentication Error', 'Please log in to follow users.');
        navigation.replace('Login');
        return;
      }

      const response = await axios.post(
        `http://192.168.43.44:3000/user/${userId}/follow`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.status === 200) {
        ...
        // Optionally, update follow state
      } else {
  ...
  const handleMessage = () => {
    // Navigate to a messaging screen or open a chat interface
    navigation.navigate('MessageScreen', { recipientId: userId });
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Follow, Message, and Edit Buttons */}
